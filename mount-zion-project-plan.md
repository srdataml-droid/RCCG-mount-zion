...
- **Single parish**, Mount Zion only — multi-tenant code structure kept
  intact underneath for possible future reuse, not actively used
...
Phase 3  AI assistant redefinition                       🟡 IN PROGRESS
...
**We are currently spanning Phases 2–4** — backend persistence is built
but not yet executed, the AI assistant has been redefined and reconnected
to a new provider, and placeholder content is being replaced with honest
markers as real information comes in.
...
## Phase 3 — AI assistant redefinition ❌ SUPERSEDED — see below
...
- Department leader names and meeting times replaced with `TO BE FILLED`
  markers, removing Gemini's invented placeholder people
...
- **Outstanding**: pastor photo still a stock placeholder
...
| PastorBot narrowed to scheduling only | Avoids unsupervised AI giving spiritual/pastoral counsel |
| Ollama Cloud over Gemini | User preference, already using it elsewhere, works the same as any cloud API |
| Link-out live stream, not embedded | Reliable, zero-maintenance, validated by how larger churches actually do it |
| `TO BE FILLED` over fake placeholder names | Prevents fabricated info accidentally shipping to production |
